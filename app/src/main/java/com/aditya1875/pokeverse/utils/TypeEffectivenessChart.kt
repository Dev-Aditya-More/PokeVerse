...
/**
 * Single source of truth for the mainline type-effectiveness chart, used across
 * team analysis, PokeDuel, and Survivor.
 *
 * Prefer this over defining private/duplicate charts in individual feature files.
 * Key: attacking type. Value: map of defending type to damage multiplier. A
 * missing entry means neutral (1x).
 */
val pokemonTypeEffectiveness: Map<String, Map<String, Float>> = mapOf(
    "normal" to mapOf(
        "rock" to 0.5f, "ghost" to 0f, "steel" to 0.5f
    ),
    "fire" to mapOf(
    ...
    "ghost" to mapOf(
        "normal" to 0f, "psychic" to 2f, "ghost" to 2f, "dark" to 0.5f
    ),
    "dragon" to mapOf(
        "dragon" to 2f, "steel" to 0.5f, "fairy" to 0f
    ),
    "dark" to mapOf(
        "fighting" to 0.5f, "psychic" to 2f, "ghost" to 2f, "dark" to 0.5f,
        "fairy" to 0.5f
    ),
    "steel" to mapOf(
...
/** Combined damage multiplier of [attackType] against a (possibly dual-typed) defender. */
fun typeEffectivenessMultiplier(attackType: String, defenderTypes: List<String>): Float {
    val row = pokemonTypeEffectiveness[attackType.lowercase()] ?: return 1f
    return defenderTypes.fold(1f) { acc, defType -> acc * (row[defType.lowercase()] ?: 1f) }
}
